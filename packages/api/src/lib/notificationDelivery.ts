...
async function logAttempt(report: NotifiableReport, trigger: NotificationTrigger, channel: "sms" | "email", ok: boolean, detail: string) {
    ...
    if (report.consumerPhone) {
      ...
      if (twilioConfig) {
        try {
          ...
          await logAttempt(report, trigger, "sms", true, `sent to ${maskContact(report.consumerPhone)}`);
        } catch (e) {
          await logAttempt(report, trigger, "sms", false, e instanceof Error ? e.message : "unknown error");
        }
      }
    }
    ...
    if (report.consumerEmail) {
      ...
      if (from) {
        try {
          ...
          await logAttempt(report, trigger, "email", true, `sent to ${maskContact(report.consumerEmail)}`);
        } catch (e) {
          await logAttempt(report, trigger, "email", false, e instanceof Error ? e.message : "unknown error");
        }
      }
    }
  } catch (e) {
